...
function WeekSelector({date, setDate}) {
    ...
    return (
        <>
            <div className={"bg-[white] grid grid-cols-7 top-[55px] px-[20px] fixed w-full h-[50px] left-0 weekSelector-border"}>
            {weekArray.map(today=>(
                <div key={today.getDate()} className={`text-center weekSelector py-[2px] ${today.getDate()===date.getDate() ? 'today' :''}`} onClick={()=>setDate(today)}>
